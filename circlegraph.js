...
function circleGraph(wakatime_data, edge_width, diameter, x_stretch, min_percent, 
    ...
    let circle_array = [];
    for (let y=0; y<parseInt(radius*2 + edge_width); y++) {
        circle_array.push([]);
    ...
    let language_info = wakatime_data["data"]["languages"];
    let current_s = 0;
    ...
    if (labels) {
        ...
        for (let lang_num=0; lang_num < language_info.length; lang_num++) {
            ...
            // for the snap effect when the label goes out of the screen
            while ((circle_array.length - min_labels_y) <= Math.floor(label_coords[1]) || Math.floor(label_coords[1]) <= min_labels_y) {
                for (let i=0; i < label_store.length; i++) {
                    let x = label_store[i];
...
async function init() {
    for (let start_angle=0; start_angle < 360; start_angle++) {
        console.clear()
